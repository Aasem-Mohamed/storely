    ...
    <div className="animate-rise">
      <section className="bg-base-200/50 border-b border-base-200">
        <div className="container mx-auto px-4 py-10">
          ...
          <h1 className="text-3xl lg:text-4xl font-medium tracking-tight">Add New Product</h1>
          <p className="text-base-content/50 mt-1">Fill in the details below to list a new product</p>
        </div>
      </section>
                  ...
                  <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-base-200">
                    ...
                    <button type="submit" disabled={formLoading} className="btn btn-primary gap-2">
                      {formLoading ? (
                        ...
                        <><HiOutlinePlus className="w-4 h-4" />Create Product</>
                      )}
                    </button>
                  </div>
                </form>
          ...
          <div className="lg:col-span-1">
            <div className="card bg-base-100 shadow-sm border border-base-200 sticky top-20">
              <div className="card-body p-6">
                ...
                <p className="text-sm text-base-content/50 line-clamp-2 mt-1">{formData.description || "Product description will appear here"}</p>
                <div className="flex items-center justify-between mt-3">
                  <span className="text-lg font-bold text-primary">
                    {formData.price ? `EGP ${Number(formData.price).toLocaleString()}` : "EGP 0"}
                  </span>
                  {formData.category && (
                    <span className="badge badge-sm badge-ghost capitalize">{formData.category}</span>
                  )}
                </div>
                {formData.stock && (
                  <p className="text-xs text-base-content/40 mt-2">{formData.stock} in stock</p>
                )}
              </div>
            </div>
          </div>
